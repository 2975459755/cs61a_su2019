...
def flatten(lst):
    ...
    for i in range(len(lst)):
        if type(lst[i])==list:
            new_lst+=flatten(lst[i])
        else:
            new_lst+=[lst[i]]
    return new_lst
    '''
    return [flatten(lst[i]) for i in range(len(lst)) if type(lst[i])==list else [lst[i]]]
    #invalid syntax :else
    '''
...
def replace_leaf(t, old, new):
    ...
    if label(t)==old and is_leaf(t):
        ...
        return tree(label(t),[replace_leaf(branches(t)[i],old,new) for i in range(len(branches(t)))])
# Mobiles
