...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        check_form(formals, len(vals), len(vals))
        child = Frame(self)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        # short-handed user-defined procedure
        lambda_def = Pair('define', Pair(target.first, Pair(Pair('lambda',\
            Pair(target.second, expressions.second)), nil)))
        return scheme_eval(lambda_def, env)
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        expr = bindings.first
        check_form(expr, 2, 2)
        if not scheme_symbolp(expr.first):
            bad_target = expr.first if isinstance(expr, Pair) else expr
            raise SchemeError('non-symbol: {0}'.format(bad_target))
        sym, val = Pair(expr.first, sym),\
                Pair(scheme_eval(expr.second.first, env), val)
        bindings = bindings.second
    return env.make_child_frame(sym, val)
    # ↓can't do parellel define
    # let_frame = env.make_child_frame(nil, nil)
    # while bindings is not nil:
    #     do_define_form(bindings.first, let_frame)
    #     bindings = bindings.second
    # return let_frame
    # END PROBLEM 15
