...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def search(place, distance, min_range, max_range):
            if place == hive or distance > max_range:
                return None
            elif place.bees and distance >= min_range:
                return random_or_none(place.bees)
            else:
                return search(place.entrance, distance+1, min_range, max_range)
        return search(self.place, 0, self.min_range, self.max_range)

        # place=self.place
        # distance=0
        # while place != hive and distance<=self.max_range:
        #     if place.bees and distance >= min_range:
        #         return random_or_none(place.bees)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while place:
            if place.ant:
                ant = place.ant
                while ant and ant.is_container:
                    buff(ant)
...
def make_slow(action, bee):
    ...
    def act(colony):
        if colony.time % 2 == 0:
            action(colony)  # move on even turns
    return act
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    new_action, original_action = effect(bee.action, bee), bee.action
    def applys(colony):
        nonlocal duration
        if duration:
            new_action(colony)
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        if self.battery:
            return max(self.damage - 0.2*distance, 0)
        else: 
            return 0
        # END Problem OPTIONAL
    ...
    @property
    def battery(self):
        return 1 - 0.05*self.insects_shot
