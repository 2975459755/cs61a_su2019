...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.operator.eval(env).apply([obj.eval(env) for obj in self.operands])
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        binds=zip(self.parameters, arguments)
        for b in binds:
            new_env[b[0]]=b[1]
        # update the environment with parameters
        expr=self.body
        while not isinstance(expr,Value):
            expr=expr.eval(new_env)
        return expr
