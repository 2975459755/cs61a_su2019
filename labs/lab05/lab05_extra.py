...
def sprout_leaves(t, vals):
    ...
    if is_leaf(t):
      ...
      for branch in branches(t):
        new_tree+=[sprout_leaves(branch,vals)]
      return new_tree
      '''
...
def add_trees(t1, t2):
    ...
    '''
    hint说让用zip函数，但没搞明白怎么用
    '''
    '''
    写了一遍就过了，虽然很难看，但很惊喜
    以免过段时间看不懂，写下思路：如果至少有一个是没有分枝的树，把node相加后直接把另一棵树的branches搬过来；
    否则，将node相加，对两棵树都有的branch进行递归，对非共有的单列出来然后一并加上（防止下标越界）
    '''